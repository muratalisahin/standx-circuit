  ...
  return (
    <section className="board">
      <div className="boardHead">
        <div>
          <span className="kicker">CANLI LİDERLİK</span>
          <strong>StandX sıralama</strong>
        </div>
        <div className="live">
          <i />
      ...
      <div className="boardTabs">
        ...
        <button type="button" className={tab === "oyun" ? "on" : ""} onClick={() => setTab("oyun")}>Oyun puanı</button>
      </div>
      ...
      {tab === "oyun" && (
        <div className="boardGame">
          <p className="boardYou">
            ...
            {" · "}
            {board.users} kayıtlı oyuncu
          </p>
          <ol className="boardList">
            {board.top.length === 0 && <li className="empty">Henüz puan yok. Oyunu bitirince hesabına yazılır.</li>}
            {board.top.map((r) => (
              <li key={`${r.name}-${r.at}`}>
                <div className={`run ${r.name === user?.name ? "on" : ""}`}>
                  <em>{r.rank}</em>
                  <b>{r.name}</b>
                  <span>{new Date(r.at).toLocaleString("tr-TR", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit" })}</span>
